...
Streszczenie	1
1. Wstęp	4
1.1 Cele, zakres i streszczenie pracy	4
2. Historia rozwoju narzędzi graficznych	5
2.1 Open source:	6
2.2 Grafika komputerowa:	8
3. Cele i założenia projektowe	11
3.1 Cele projektowe	11
3.2 Wymagania funkcjonalne i pozafunkcjonalne	11
3.2.1 Wymagania funkcjonalne:	11
3.2.2 Wymagania pozafunkcjonalne:	11
3.2.3 Wymagania Techniczne	12
3.3 Analiza profilu użytkownika	12
3.4 Przegląd istniejących rozwiązań	12
3.5  Wizja realizacji projektu	12
3.5.1 Schemat działania aplikacji - Diagramy UML	12
3.5.2 Koncepcja wyglądu UI aplikacji	12
3.5.3 Zadania projektowe	13
4. Tworzenie aplikacji mobilnych - analiza zagadnień teoretycznych	13
4.1 Android – najważniejsze cechy systemu	13
4.2 Aplikacje mobilne – typy aplikacji, proces tworzenia	14
4.2.1 Aplikacje natywne	14
4.2.2 Aplikacje Hybrydowe	14
4.3 Język programowania i środowisko programistyczne	14
4.4 Biblioteki i frameworki	14
4.4.1 Biblioteki	14
4.4.2 Frameworki	15
4.5 Narzędzia graficzne	15
4.6 Testowanie aplikacji	16
4.7 Zarządzanie projektem informatycznym	16
5. Proces implementacji i dokumentacja techniczna	17
5.1 Konfiguracja środowiska i implementacja architektury	17
5.2 Etap 1…….	17
5.3 Etap 2…..	17
5.4 Projekt graficzny aplikacji	17
5.5 Testowanie aplikacji	17
5.6 Dokumentacja techniczna projektu	17
Wnioski końcowe	18
Bibliografia	19
...
Streszczenie:
1 nieważne
2 jak działa ludzkie oko
3 dlaczego grafika komputerowa działa inaczej
4 czym jest grafika raymarching
5 czym jest grafika wektorowa
5.5 transformacje użyte w projekcie
6 proces transformacji macierzowych
7 kolor i tekstury
8 filtrowanie tekstur(anti-aliasing)
9 czym są obiekty
10 czym są macierze
11 czym są shadery
12 fragment shader
13 czym są tekstury
14 czym są macierze
15 dlaczego użycie macierzy jest kożystne
16 typy macierzy - modelu, świata, kamery, frustum
17 płaszczyzny i przecięcie z prostymi
18 opis jak działa przypisywanie obrazów w mojej aplikacji
19 wektory normalne
20 czym jest openCL
21 opengles pipeline
22 opis pipeline

Grafika komputerowa znacząco różni się od świata rzeczywistego. Celem programistów tworzących programy symulujące przestrzeń 3D jest zasymulowanie rzeczywistych fenomenów otaczającego nas świata w jak najbardziej przekonujący sposób, jednak symulacja świata wirtualnego zgodnie z zasadami świata rzeczywistego pozostaje wciąż w obrębie fikcji. Dlatego też programiści zajmujący się grafiką 3D wynaleźli całą gamę algorytmów, które mają w uproszczony sposób zasymulować zjawiska fizyczne. Przykładowo, wiemy że ludzkie oko rejestruje widziany przez nas obraz poprzez pochłanianie odbitych w stronę oka fali światła widzialnego. Jednak obliczanie trajektorii promieni świelnych, poza tymi które trafiły by do kamery było by z punktu widzenia wydajności algorytmu wielką stratą zasobów komputera - w końcu takie promienie nie wpływają w żaden sposób na uzyskany przez kamerę obraz. Mamy tu paradoks drzewa przewracającego się w lesie - jeśli nie ma w lesie nikogo kto mógłby usłyszeć upadek drzewa, czy możemy powiedzieć że upadło z hukiem? Z perspektywy programisty grafiki 3D, dźwięk upadającego drzewa w lesie, czy odchodząc od metafory, promienie świetlne nie rejestrowane przez kamerę, mogły by równie dobrze nie istnieć. Liczy się tylko to co rejestruje kamera, to co zostanie wyświetlone na ekranie. Dlatego też, aby zminimalizować ilość obliczeń, w dwóch najpopularniejszych podejściach do wyświetlania przestrzeni 3D, nie używa się symulacji promieni świetlnych wychodzących z źródłą światła. 

Zamiast tego w grafice Raymarching, stosuje się odwrócony paradygmat - aby mieć pewność że obliczenia skupiają się wyłącznie na promieniach światła rejestrowanych przez kamerę, symulowane promienie wychodzą nie z źródła światła, a z samej kamery. w najprostszej wersji tego algorytmu, po osiągnięciu minimalnej odległości do obiektu w przestrzeni 3D, funkcja obliczająca trajektorię promienia potwierdza zderzenie z obiektem, a do piksela z którego wyszedł promień przypisywany jest kolor obiektu. Promienie które nie zarejestrują kolizji z żadnym obiektem, po osiągnięciu maksymalnej długości lub cykli przedłużających promień, zwracają informację o braku kolizji. Wtedy piksel z którego wyszedł taki promień otrzyma domyślny kolor tła. Jest to oczywiście uproszczony opis działania algorytmu Raymarching, jednak nawet w takim uproszczeniu, oczywistym jest że ograniczenie liczby promieni do tych które rejestrowane są na kamerze, a także ograniczenie się do prostoliniowych promieni z pominięciem odbić, pozwala drastycznie przyspieszyć działanie algorytmu.

Natomiast w najczęściej stosowanym algorytmie wyświetlania, użytym także w Wirtualnej Galerii - grafice wektorowej - zupełnie odchodzi się od obliczeń promieni. Wykorzystując fakt iż algorytm wyświetlający scenę 3D ma dostęp do pozycji wszystkich obiektów w scenie, używa się przekształceń macierzowych, które odpowiednio w stosunku odległości od kamery kurczą odległe obiekty, jednocześnie powiększając bliższe. Równocześnie geometria tych obiektów zostaje spłaszczona tak że ostatecznie wierzchołki wszystkich obiektów znajdują się na jednej płaszczyźnie, prostopadłej do kierunku skierowania kamery. Wtedy kwestia obliczenia koloru pojedynczego piksela staje się trywialna. Wystarczy znaleźć odpowiadającą pozycję w spłaszczonym widoku, i sprawdzić czy znajduje się ona wewnątrz jakiegoś obiektu, a jeśli tak to jaki jest jego kolor. W przypadku gdy za kolor bazowy obiektu odpowiada tekstura, sposób przypisania koloru do piksela wewnątrz obiektu jest trochę bardziej skąplikowany. Należy wtedy odwrócić macierz użytą do przekształcenia obiektu i za jej pomocą obliczyć pozycję punktu odpowiadającego pozycji piksela, na teksturze przypisanej do obiektu. Wtedy obliczenie odpowiadającego koloru to kwestia zastosowanego algorytmu. Najprostszym algorytmem jest tzw. Nearest czyli znalezienie najbliższego do punktu teksela i przypisanie wartości koloru teksela pikselowi. Niestety, rozwiązanie to powoduje niedokładne przypisanie koloru dla odległych i pochyłych powierzchni, tzw. Aliasing . Aby uniknąć aliasingu stosuje się różnorodne metody, wszystkie nazwane ze względu na przeznaczenie metodami "Anti-Aliasing". Jednym ze sposobów walki z aliasingiem jest zastosowanie mipmapingu, czyli tych samych tekstur w różnych wielkościach, wybieranych w zależności od odległości obiektu od kamery. Wadą tego rozwiązania jest jednak rozmycie odległych obiektów. Jeszcze inne podejście zakłada użycie algorytmów które w bardziej dokładny sposób obliczają kolor w punkcie na teksturze, np. przez uśrednienie koloru pobliskich tekseli. Do grona takich metod zaliczamy min. filtrowanie bilinearne czy trilinearne. Wadą tego typu rozwiązań jest znaczne zwiększenie ilości obliczeń w porównaniu do metody Nearest. Istnieją jeszcze inne metody filtrowania tekstur, min. szeroko wykorzystywana w dzisiejszych czasach metoda SSAA - super sampling anti aliasing, jednak działanie takich algorytmów wykracza poza zakres tej pracy. Więcej na temat filtrowania tekstur można znaleść w załączonych odnośnikach <tutaj odnośniki>.
...
*OpenGL ES Pipeline - chyba jest uproszczone. na: https://gamedev.stackexchange.com/questions/150/what-is-the-difference-between-opengl-es-and-opengl znalazłem info że w OpenGL ES nie ma "Fixed Pipeline".


Kolejne etapy różnią się nazwą w zależności od silnika graficznego. W zależności czy jest to Direct3D, OpenGL, Vulcan, czy inny silnik graficzny, niektóre z wymienionych etapów mogą być dostępne pod inną nazwą, pominięte, lub niedostępne do edycji przez użytkownika, a nawet różnić się kolejnością. Przykładowo to co w OpenGL dostępne jest pod nazwą Fragment Shader, w Direct3D znane jest jako Pixel Shader. Dlatego poniżej opisuję tylko jak proces ten wygląda w OpenGL ES, i tylko te etapy które były użyte w projekcie (nie wymieniam ukrytych etapów, których OpenGL ES nie udostępnia do modyfikacji):
...
8) użycie Fragment Shadera do obliczenia koloru pikseli na podstawie danych obliczonych w poprzednim etapie. To w tym etapie zachodzi proces rasteryzacji
